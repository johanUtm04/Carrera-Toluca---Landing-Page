...
<body class="cuerpo-corporativo">
    ...
    <header class="cabecera-principal">
        <div class="cabecera-contenedor">
            ...
            <nav class="cabecera-acciones" aria-label="Autenticación de Usuario">
                @if(Auth::check())
                    <a href="{{ route('dashboard') }}" class="johan-btn-nav-profile" title="Ir a mi panel de atleta">
                        BIENVENIDO, {{ Str::upper(Str::limit(Auth::user()->name, 12)) }} »
                    </a>
                @else
                    <a href="{{ route('login') }}" class="johan-btn-nav-auth" title="Iniciar sesión en el portal">
                        INICIAR SESIÓN »
                    </a>
                @endif
            </nav>
